...
if PY3:
	from urllib.parse import quote_plus, unquote_plus, unquote, urlparse	
else:
...
def start(signin):
	if username == "":
		...
		passw = tools.keypopup('Introduzca su contraseña')
		...
		if login_data == 0:
			...
			line2 = "ienvenido a nuestro servicio "+ADDON_NAME
...
def stream_video(url,iconimage):
	...
	if not '.mp4' in url and not '.avi' in url and not '.rmv' in url and not 'rmvb' in url:
		...
		check = False
	if f4m == 'true' and check == True:
		# dialog = xbmcgui.Dialog()
		# ok = dialog.ok('Kodi', 'f4m ok')			
		name = 'XCUI'
		url = ts_to_m3u8(url)
		url = url + headers
		if '.m3u8' in url and not 'plugin' in url:
			url = 'plugin://plugin.video.f4mTester/?streamtype=HLSRETRY&name='+quote_plus(str(name))+'&iconImage='+quote_plus(str(iconimage))+'&thumbnailImage='+quote_plus(str(iconimage))+'&url='+quote_plus(url)
		# elif not '.mp4' in url and not '.mkv' in url and not '.avi' in url and not '.rmvb' in url and not '.mp3' in url and not '.wmv' in url and not '.wma' in url and not '.ac3' in url and not 'plugin' in url:
		# 	url = 'plugin://plugin.video.f4mTester/?streamtype=TSDOWNLOADER&name='+quote_plus(str(name))+'&iconImage='+quote_plus(str(iconimage))+'&thumbnailImage='+quote_plus(str(iconimage))+'&url='+quote_plus(url)
		if 'plugin://' in url:
...
def settingsmenu():
	...
	tools.addDir('F4mplayer %s'%f4m_,'F4M',10,icon,background,f4m_)		
...
def addonsettings(url,description):
	...
	if	 url =="clearcache":
		...
		if 'ON' in description:
			xbmcaddon.Addon().setSetting('f4m','false')
			xbmc.executebuiltin('Container.Refresh')
		else:
			xbmcaddon.Addon().setSetting('f4m','true')
			xbmc.executebuiltin('Container.Refresh')
	elif url =="META":
		if 'ON' in description:
...
def accountinfo():
	...
	tools.addDir('[B][COLOR white]Estado de la cuenta: [/COLOR][/B] %s'%status,'','',icon,background,'')
