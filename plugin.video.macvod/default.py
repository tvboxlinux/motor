...
def main_list(params):
    ...
    plugintools.add_item( action="mac", title="[COLOR orange]PORTAL / STB / MAC[/COLOR]", thumbnail = thmb_ver_stb, fanart= backgr,page="",url="",folder=True )
...
def mac(params):
    ...
    plugintools.add_item(action="tulista", title="[COLOR orange]STB / MAC - Servidor aleatorio[/COLOR]  [COLOR white]("+nat+" pruebas por dia)[/COLOR]",thumbnail=thmb_ver_stb, fanart="https://e00-elmundo.uecdn.es/assets/multimedia/imagenes/2017/05/23/14955340432162.jpg",  url= "https://pastebin.com/raw/ktiz5e2M",folder= True )    
    plugintools.add_item( action="macpastebinx", title="[COLOR lime]STB / MAC - Portal[/COLOR]", thumbnail = thmb_ver_stb, fanart= backgr,page="",url="",folder=True )
    plugintools.add_item( action="macx", title="[COLOR orange]STB / MAC - Tu Portal[/COLOR]", thumbnail = thmb_ver_stb, fanart= backgr,page="",url="",folder=True )
    plugintools.add_item(action = "main_list" , title = "Volver", thumbnail =thmb_ver_xc, fanart = backgr, folder = True )
...
def xtreamcodes(params):
    ...
    plugintools.add_item( action="ipkoditv_enigmax", title="[COLOR lime]Xtream Codes - Tu portal[/COLOR]", thumbnail = thmb_ver_xc, fanart= backgr,page="",url="",folder=True )
...
def macpastebin(params):
    ...
    plugintools.add_item(action="ver_canales",    title="Listados de Portales",thumbnail=thmb_ver_canales,fanart="https://i.ytimg.com/vi/_7bFXWNfXTY/maxresdefault.jpg",folder= True )            
    plugintools.add_item(action="", thumbnail=thmb_nada,title="[COLOR gray]Configuracion actual--------------------------------------------------------------------------[/COLOR]",folder= False )
    plugintools.add_item(action="cambio_servidor",    title="Conectado al servidor:   "+escogido,thumbnail=thmb_cambio_servidor,fanart="https://www.zooplus.es/magazine/wp-content/uploads/2018/04/fotolia_169457098.jpg",folder= True )               
...
def lista2(params):
    ...
    pb.create('Espera, los canales se estan cargando ','')
    count=40;pn=1;data=[]
    while pn <= int(count):
...
def tulista(params):
    ...
    if fec==hoy and maximo==False:
        dialog = xbmcgui.Dialog()
        ret = dialog.select('Selecciona opcion', ['[COLOR red]CREAR LISTA NUEVA [/COLOR]       [ You have left [COLOR green]'+str(nat)+'[/COLOR] attempt ]', '[COLOR white]Seguir com mi lista de HOY[/COLOR]'])
