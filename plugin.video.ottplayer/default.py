...
def start(signin):
	if username == "":
		dns = tools.keypopup('Ingrese su servidor ejemplo: http://servidor:puerto')
		usern = tools.keypopup('Usuario')
		passw = tools.keypopup('Contraseña')
		...
		if login_data == 0:
			...
			line2 = "Vuelva a ingresar" 
			line3 = "" 
			xbmcgui.Dialog().ok('Attention', line1+'\n'+line2+'\n'+line3)
			start()
		else:
			line1 = "Conectado correctamente al servidor"
			line2 = "Bienvenido "+ADDON_NAME
			line3 = ('[B][COLOR lime]%s[/COLOR][/B]'%usern)
...
def home():
	tools.addDir('Información de la cuenta','url',6,iconaccount,background,'')
	...
	tools.addDir('Busqueda','url',5,iconsearch,background,'')
	tools.addDir('Ajustes','url',8,iconsettings,background,'')
...
def settingsmenu():
	...
	tools.addDir('Editar configuracion avanzada','ADS',10,icon,background,'')
	tools.addDir('Metadatos de peliculas %s'%META,'META',10,icon,background,META)
...
def addonsettings(url,description):
	...
	if	 url =="clearcache":
		...
		dialog = xbmcgui.Dialog().select('Seleccione su dispositivo o el mas cercano', ['Open AutoConfig','Fire TV Stick ','Fire TV','1GB Ram or Lower','2GB Ram or Higher','Nvidia Shield'])
		if dialog==0:
			advancedsettings('auto')
			tools.ASln()
		elif dialog==1:
		...
		if 'ON' in description:
			pas = tools.keypopup('Ingrese la contraseña de canales de adultos:')
...
def adult_set():
	dialog = DIALOG.yesno(ADDON_NAME,'¿Te gustaria ocultar el menu de adultos? \nSiempre puedes cambiar esto en la configuracion mas adelante.')
	if dialog:
		...
		pass
	dialog = DIALOG.yesno(ADDON_NAME,'¿Le gustaría proteger con contraseña el contenido para adultos? \nSiempre puedes cambiar esto en la configuracion mas adelante.')
	if dialog:
		control.setSetting('xxx_pwset','true')
		adultpw = tools.keypopup('Introducir la contraseña')
...
def accountinfo():
	...
	tools.addDir('[B][COLOR white]Fecha de termino:[/COLOR][/B] '+expiry,'','',icon,background,'')
	tools.addDir('[B][COLOR white]Estado de la cuenta :[/COLOR][/B] %s'% parse['user_info']['status'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]Equipos conectados:[/COLOR][/B] '+ parse['user_info']['active_cons'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]Maximo de conexiones:[/COLOR][/B] '+ parse['user_info']['max_connections'],'','',icon,background,'')
