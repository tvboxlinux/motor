...
def menu(params):
	...
	add_item( action="account",portal=params['url'], title="Mi cuenta" , thumbnail=os.path.join(artPath ,"logo.png"), fanart=os.path.join(artPath ,"background.png") , folder=True)
	add_item( action="play", portal=params['url'], title ="Canales" , thumbnail=os.path.join(artPath ,"logo.png"), fanart=os.path.join(artPath ,"background.png") , folder=True)
	add_item( action="vod_categories",   portal=params['url'], title="Peliculas" , thumbnail=os.path.join(artPath ,"logo.png"), fanart=os.path.join(artPath ,"background.png") , folder=True)
	...
	add_item( action="cleargroups", portal=params['url'], title="Restablecer los grupos" , thumbnail=os.path.join(artPath ,"logo.png"), fanart=os.path.join(artPath ,"background.png") , folder=False)
	add_item( action="settings", title="Ajustes" , thumbnail=os.path.join(artPath ,"logo.png"), fanart=os.path.join(artPath ,"background.png"), folder=False)
	set_view( LIST)
...
def reset_portal_settings(number):
	set_setting("website" + number, "http://"), set_setting("port" + number, ""), set_setting("username" + number, ""), set_setting("password" + number, "")
def settings(params):
...
def live_categories(params):
    ...
    indicies = selector(titles, "Elija los grupos de carpetas a reproducir", True)
    group = []
    if indicies:
        for i in indicies:
            group.append(ids[i])
...
def portals(params):
	for i in range(13):
		if get_setting("website"+str(i)) != "http://" and get_setting("website"+str(i)) != "http://":
	...
	add_item( action="cleargroups", title="Restablecer todos los grupos" , thumbnail=os.path.join(artPath ,"logo.png"), fanart=os.path.join(artPath ,"background.png") , folder=False)
	add_item( action="settings", title="Ajustes" , thumbnail=os.path.join(artPath ,"logo.png"), fanart=os.path.join(artPath ,"background.png"), folder=False)
	set_view( LIST)
...
def cleargroups(params):
	...
	if params.get("portal") is None:
		...
		if message_yes_no("Grupos del portal: "+params.get("portal")+" restablecen a los predeterminados?"):
