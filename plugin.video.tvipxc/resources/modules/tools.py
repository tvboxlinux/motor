...
def clear_cache():
	...
	confirm=xbmcgui.Dialog().yesno("Por favor confirmar","Confirme que desea eliminar el caché de su servidor")
	if confirm:
				...
				if file_count > 0:
						...
						for d in dirs:
							try:
								...
								pass
		LogNotify("[COLOR {0}]{1}[/COLOR]".format(COLOR1, ADDON_NAME), '[COLOR {0}]Caché borrado exitosamente![/COLOR]'.format(COLOR2))
...
def killxbmc():
	killdialog = xbmcgui.Dialog().yesno('Forzar cierre', '[COLOR white]Estás a punto de cerrar tvipxc', 'Te gustaria continuar?[/COLOR]', nolabel='[B][COLOR red] No Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Forzar cierre[/COLOR][/B]')
	if killdialog:
		os._exit(1)
	else:
		home()
