...
def start(signin):
	if username == "":
		...
		if login_data == 0:
			line1 = "Inicio de sesion incorrecto"
			line2 = "Vuelva a ingresar sus datos" 
			line3 = "" 
			xbmcgui.Dialog().ok('Attention', line1+'\n'+line2+'\n'+line3)
			start()
		else:
			line1 = "Conexion exitosa"
			line2 = "Bienvenido a nuestro servicio "+ADDON_NAME
...
def home():
	tools.addDir('Informacion de la cuenta','url',6,iconaccount,background,'')
	tools.addDir('Television','live',1,iconlive,background,'')
	...
	tools.addDir('Peliculas','vod',3,iconMoviesod,background,'')
	tools.addDir('Buscar por titulo','url',5,iconsearch,background,'')
	tools.addDir('Configuraciones','url',8,iconsettings,background,'')
...
def settingsmenu():
	...
	tools.addDir('Editar configuracion del buffer','ADS',10,icon,background,'')
	tools.addDir('Metadatos del servidor %s'%META,'META',10,icon,background,META)
	tools.addDir('Ocultar contenido para adultos %s'%xxx,'XXX',10,icon,background,xxx)
	tools.addDir('Eliminar la cuenta','LO',10,icon,background,'')
...
def addonsettings(url,description):
	...
	if	 url =="clearcache":
		...
		dialog = xbmcgui.Dialog().select('Editar configuracion avanzada', ['Configuracion automatica','Habilitar Fire TV Stick AS','Habilitar Fire TV AS','Habilitar 1GB Ram or Lower AS','Enable 2GB Ram or Higher AS','Habilitar Nvidia Shield AS','Disable AS'])
		if dialog==0:
		...
		dialog = xbmcgui.Dialog().select('Seleccione su dispositivo o el mas cercano en memoria', ['Configuracion automatica','Fire TV Stick ','Fire TV','1GB Ram or Lower','2GB Ram or Higher','Nvidia Shield'])
		if dialog==0:
			advancedsettings('auto')
			tools.ASln()
		elif dialog==1:
		...
		dialog = xbmcgui.Dialog().yesno(ADDON_NAME,'¿Le gustaria que configuremos la guia de TV para usted?')
		if dialog:
			pvrsetup()
			xbmcgui.Dialog().ok(ADDON_NAME, 'Integracion de PVR completa, reinicie Kodi para que los cambios surtan efecto')
	elif url =="Itv":
			xbmc.executebuiltin('Instalacion completa (pvr.iptvsimple)')
	elif url =="ST":
		speedtest.speedtest()
	elif url =="META":
...
def adult_set():
	dialog = DIALOG.yesno(ADDON_NAME,'Te gustaria ocultar los canales para adultos \nPuedes cambiar esto en la configuracion mas adelante.')
	if dialog:
		...
		pass
	dialog = DIALOG.yesno(ADDON_NAME,'Le gustaria proteger con contraseña el contenido para adultos \nPuedes cambiar esto en la configuracion mas adelante.')
	if dialog:
		control.setSetting('xxx_pwset','true')
		adultpw = tools.keypopup('Introducir la contraseña a eleccion')
...
def accountinfo():
	...
	if max_connection == 'None':
		...
		max_connection = 'Ilimitado'		
	tools.addDir('[B][COLOR white]Usuario: [/COLOR][/B] '+str(parse['user_info']['username']),'','',icon,background,'')
	tools.addDir('[B][COLOR white]Contraseña: [/COLOR][/B] '+str(parse['user_info']['password']),'','',icon,background,'')
	tools.addDir('[B][COLOR white]Termino del servicio: [/COLOR][/B] '+expiry,'','',icon,background,'')
	tools.addDir('[B][COLOR white]Estado de la cuenta: [/COLOR][/B] %s'% parse['user_info']['status'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]Conexiones activas: [/COLOR][/B] '+ str(parse['user_info']['active_cons']),'','',icon,background,'')
	tools.addDir('[B][COLOR white]Conexiones permitidas: [/COLOR][/B] '+ max_connection,'','',icon,background,'')
	...
	tools.addDir('[B][COLOR white]Ottlinux version: [/COLOR][/B] '+str(KODIV),'','',icon,background,'')
