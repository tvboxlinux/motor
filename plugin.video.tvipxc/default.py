...
live			  = os.path.join(MEDIA,   'live.png')
...
def addonsettings(url,description):
	...
	if	 url =="clearcache":
		...
		dialog = xbmcgui.Dialog().select('Editar configuración avanzada', ['Configuracion automatica','Habilitar Fire TV Stick AS','Habilitar Fire TV AS','Habilitar 1GB Ram or Lower AS','Habilitar 2GB Ram or Higher AS','Habilitar Nvidia Shield AS','Deshabilitar y borrar toda la configuracion'])
		if dialog==0:
		...
		dialog = xbmcgui.Dialog().select('Seleccione su dispositivo o el mas cercano en memoria', ['Configuracion automatica','Fire TV Stick ','Fire TV','1GB de memoria o menos','2GB memoria o mas','Nvidia Shield'])
		if dialog==0:
			advancedsettings('auto')
			tools.ASln()
		elif dialog==1:
...
def accountinfo():
	...
	if not expiry=="":
		...
		expiry = 'Unlimited'
	tools.addDir('[B][COLOR white]Usuario :[/COLOR][/B] '+parse['user_info']['username'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]Contraseña :[/COLOR][/B] '+parse['user_info']['password'],'','',icon,background,'')
	...
	tools.addDir('[B][COLOR white]Conexiones activas:[/COLOR][/B] '+ parse['user_info']['active_cons'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]Conexiones maximas:[/COLOR][/B] '+ parse['user_info']['max_connections'],'','',icon,background,'')
	tools.addDir('[B][COLOR white]IP Local:[/COLOR][/B] '+ tools.getlocalip(),'','',icon,background,'')
	tools.addDir('[B][COLOR white]IP Externa:[/COLOR][/B] '+ tools.getexternalip(),'','',icon,background,'')
